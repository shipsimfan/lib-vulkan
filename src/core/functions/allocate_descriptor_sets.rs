...
use crate::{VK_VERSION_1_0, VkDescriptorPoolCreateInfo, VkDescriptorType};
...
/// sum of all the `descriptor_count` members of each element of VkDescriptorPoolCreateInfo::pPoolSizes with a type equal to that type.
///
...
///
/// [`VkAllocateDescriptorSets`] can be used to create multiple descriptor sets. If the creation of any of those descriptor sets fails, then the implementation must destroy all successfully created descriptor set objects from this command, set all entries of the `descriptor_sets` array to VK_NULL_HANDLE and return the error.
///
/// # Valid Usage (Implicit)
///  - device must be a valid VkDevice handle
///  - `allocate_info` must be a valid pointer to a valid VkDescriptorSetAllocateInfo structure
///  - `descriptor_sets` must be a valid pointer to an array of `allocate_info`->descriptorSetCount VkDescriptorSet handles
///  - The device must have been created with at least 1 queue
///  - `allocate_info`->descriptorSetCount must be greater than 0
///
/// # Return Codes
///
