use crate::{
    VkAllocationCallbacks, VkDevice, VkResult,
...
use crate::{
    VK_NULL_HANDLE, VkSampleCountFlag,
...
///  - `create_info` must be a valid pointer to a valid [`VkSwapchainCreateInfoKhr`] structure
///  - If `allocator` is not [`null`], `allocator` must be a valid pointer to a valid
///    [`VkAllocationCallbacks`] structure
///  - `swapchain` must be a valid pointer to a [`VkSwapchainKhr`] handle
